...
void ofApp::setup() {
	...
	auto span = 20;
	for (int base_x = -240; base_x <= 240; base_x += 240) {

		for (int base_y = -240; base_y <= 240; base_y += 240) {

			for (int x = -80; x <= 80; x += span) {

				for (int y = -80; y <= 80; y += span) {

					this->location_list.push_back(glm::vec3(base_x + x, base_y + y, 100));
				}
			}
...
void ofApp::update() {
	...
	if (ofGetFrameNum() % (frame_span * 10) < frame_span * 7) {
